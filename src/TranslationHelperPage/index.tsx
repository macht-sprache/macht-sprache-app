        ...
        <>
            ...
            <div className={s.analysisContainer}>
                <p>
                    ...
                    <SensibleWord translationList={dummyTranslationRace}>white fragility</SensibleWord>. Though{' '}
                    <SensibleWord translationList={dummyTranslationRace}>white fragility</SensibleWord> is triggered by
                    discomfort and anxiety, it is born of superiority and entitlement.{' '}
                    <SensibleWord translationList={dummyTranslationRace}>White fragility</SensibleWord>
...
function SensibleWord({ children, translationList }: { children: React.ReactNode; translationList: TranslationList }) {
    ...
    const tooltip = (
        <div className={s.overlay}>
            <span className={s.overlayWord}>{translationList.word}</span>: {translationList.definition}
            <h4 className={s.overlayHeading}>Possible translations</h4>
            <ul className={s.overlayTranslationList}>
                {translationList.translations.map((translation, index) => (
                    <li key={index} className={s.overlayTranslation}>
                        <span className={s.overlayWord}>{translation.word}</span>: {translation.definition}
                    </li>
                ))}
            </ul>
            <a href={translationList.link}>More information</a>
        </div>
    );
    ...
    return (
        <Tooltip
            ...
            mouseLeaveDelay={0.2}
        >
            <span className={clsx(s.sensibleWord, { [s.sensibleWordHovered]: isVisible })}>{children}</span>
        </Tooltip>
    );
}
...
function Form({ onSubmit }: { onSubmit: () => void }) {
    ...
    const [text, setText] = useState(
        'Led to factors like race, development and location - to who does and who doesn’t make a ‚relatable’ victim of terror. I conceptualize this process as white fragility. Though white fragility is triggered by discomfort and anxiety, it is born of superiority and entitlement. White fragility is not weakness per se. In fact, it is a powerful means of white racial control and the protection of white advantage.'
    );
